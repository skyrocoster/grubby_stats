...
hero = st.sidebar.multiselect(
    ...
    default=player_matches["hero_name"].head(10).unique(),
)
game_mode = st.sidebar.multiselect("Select Lobby Type", options=[0, 7], default=7)
rank = st.sidebar.multiselect("Select Rank", options=player_matches['average_rank'].unique())

player_matches_sel = player_matches.query(
    "hero_name == @hero & lobby_type == @game_mode"
)
